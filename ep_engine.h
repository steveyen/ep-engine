...
public:
                      ...
                      void *engine_specific)
    {
        ...
        TapClientConnection *tc = static_cast<TapClientConnection*>(engine_specific);

        if (status == ON_CONNECT) {
            assert(userdata != NULL);
            assert(nuserdata != NULL);
            assert(flags != NULL);
            ...
            if (tc != NULL) {
                ...
                if ((backfill = getenv("MEMCACHED_TAP_BACKFILL_AGE")) != NULL) {
                    ...
                    *nuserdata = sizeof(backfillage);
                    memcpy(userdata, &backfillage, *nuserdata);
                }
        ...
        { 
            ...
            for (iter = clientTaps.begin(); iter != clientTaps.end(); iter++) {
                ...
                if (tc->failed && --tc->retry_interval == 0) {
                    tc->failed = false;
                    serverApi->core->tap_connect(tc->peer.c_str(), CONNECTION_TIMEOUT, (void *)tc);
                }
            }
        }
    }
...
private:
    ...
    void tapConnect(std::string &peer) {
        ...
        if (!found) {
            ...
            serverApi->core->tap_connect(peer.c_str(), CONNECTION_TIMEOUT, (void *)tc);
        }
    }
